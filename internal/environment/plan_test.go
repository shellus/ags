...
func TestPreparePlanPlansUnmanagedTakeover(t *testing.T) {
	...
	build := BuildResult{Root: filepath.Join(home, "stage"), Instruction: []byte("rules"), Agents: map[agent.Name]RenderedAgent{
		agent.Codex: {Name: agent.Codex, Stage: stage, Skills: []string{"demo"}, DisabledSkills: []string{"openai-docs"}},
	}}
	...
	if !item.ConfigChanged || len(item.DisabledSkills) != 1 || item.DisabledSkills[0] != "openai-docs" {
		t.Fatalf("Codex config plan = %#v", item)
	}
	if !plan.HasChanges() {
